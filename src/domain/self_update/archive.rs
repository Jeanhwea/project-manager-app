...
fn extract_from_tar_gz(data: &[u8], bin_name: &str) -> Result<Vec<u8>> {
    ...
    for entry in archive.entries().map_err(|e| SelfUpdateError::TarRead { source: e })? {
        let mut entry = entry.map_err(|e| SelfUpdateError::TarEntry { source: e })?;
        let path = entry.path().map_err(|e| SelfUpdateError::TarEntryPath { source: e })?;
...
fn extract_from_zip(data: &[u8], bin_name: &str) -> Result<Vec<u8>> {
    ...
    let mut archive = zip::ZipArchive::new(cursor).map_err(|e| SelfUpdateError::ZipRead { source: Box::new(e) })?;

    for i in 0..archive.len() {
        let mut file = archive
            .by_index(i)
            .map_err(|e| SelfUpdateError::ZipEntry { source: Box::new(e) })?;
