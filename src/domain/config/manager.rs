...
impl MultiSourceConfigManager {
    ...
    fn load_from_env(&self) -> Result<AppConfig> {
        ...
        if let Ok(default_protocol) =
            env::var(format!("{}GITLAB_DEFAULT_PROTOCOL", self.env_prefix))
        {
    ...
    fn merge_configurations(&self, file_config: AppConfig, env_config: AppConfig) -> AppConfig {
        ...
        merged.gitlab.server = env_config.gitlab.server.or(file_config.gitlab.server);

        merged.gitlab.token = env_config.gitlab.token.or(file_config.gitlab.token);
...
impl ConfigManager for MultiSourceConfigManager {
    ...
    fn save(&self, config: &AppConfig) -> Result<()> {
        ...
        let content =
            serde_json::to_string_pretty(config).map_err(|e| ConfigError::ParseError(e))?;
...
mod tests {
    ...
    fn test_file_config_loading() {
        ...
        let manager =
            MultiSourceConfigManager::with_settings(Some(config_path), "TEST_".to_string());
    ...
    fn test_env_config_loading() {
        ...
        let manager = MultiSourceConfigManager::with_settings(None, "TEST_".to_string());

        let env_config = manager.load_from_env().unwrap();

        assert_eq!(
            env_config.git.skip_push_hosts,
            vec!["host1.com", "host2.com"]
        );
        assert_eq!(env_config.git.default_protocol, "ssh");
        assert_eq!(
            env_config.gitlab.server,
            Some("https://test.gitlab.com".to_string())
        );
    ...
    fn test_cli_overrides() {
        ...
        overrides.insert(
            "gitlab.server".to_string(),
            "https://cli.gitlab.com".to_string(),
        );
        ...
        assert_eq!(
            overridden_config.gitlab.server,
            Some("https://cli.gitlab.com".to_string())
        );
    ...
    fn test_config_merging() {
        ...
        assert_eq!(
            merged.gitlab.server,
            Some("https://env.gitlab.com".to_string())
        );
