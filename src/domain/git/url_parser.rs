...
pub struct RemoteUrlParser;

impl RemoteUrlParser {
    ...
    pub fn extract_host_and_path(url: &str) -> Option<(String, String)> {
        ...
        let protocol = Self::detect_protocol(url).ok()?;

        let (normalized, separator) = Self::normalize_url(url, &protocol);
...
mod tests {
    ...
    fn test_detect_protocol_ssh() {
        assert_eq!(
            RemoteUrlParser::detect_protocol("git@github.com:user/repo.git").unwrap(),
            GitProtocol::Ssh
        );
        assert_eq!(
            RemoteUrlParser::detect_protocol("ssh://git@github.com/user/repo.git").unwrap(),
            GitProtocol::Ssh
        );
    }
    ...
    fn test_detect_protocol_https() {
        assert_eq!(
            RemoteUrlParser::detect_protocol("https://github.com/user/repo.git").unwrap(),
            GitProtocol::Https
        );
    }
    ...
    fn test_detect_protocol_http() {
        assert_eq!(
            RemoteUrlParser::detect_protocol("http://github.com/user/repo.git").unwrap(),
            GitProtocol::Http
        );
    }
    ...
    fn test_detect_protocol_heuristic_ssh() {
        assert_eq!(
            RemoteUrlParser::detect_protocol("user@host:path").unwrap(),
            GitProtocol::Ssh
        );
    }
    ...
    fn test_detect_protocol_invalid() {
        assert!(RemoteUrlParser::detect_protocol("").is_err());
        assert!(RemoteUrlParser::detect_protocol("invalid-url").is_err());
    }
    ...
    fn test_extract_host_and_path_invalid() {
        assert!(RemoteUrlParser::extract_host_and_path("").is_none());
        assert!(RemoteUrlParser::extract_host_and_path("invalid-url").is_none());
    }
}
