...
## Why
当前 release/sync/doctor 等命令中，git 操作散落在各处，缺乏统一的执行流程。需要引入"上下文收集 → 执行计划 → 执行命令"三阶段模式，使 dry_run 时只展示计划不执行命令，同时让所有命令遵循一致的流程。
...
- 重构 `release.rs` 使用三阶段流程
- 重构 `sync.rs` 使用三阶段流程
- 重构 `doctor.rs` 使用三阶段流程
- 移除 `command.rs` 中已迁移到 `GitOperation` 的高级方法（add/commit/create_tag 等）
...
- Affected code: `src/commands/` (release.rs, sync.rs, doctor.rs)
...
### Requirement: 三阶段流程统一
所有 git 写命令 SHALL 遵循"上下文收集 → 执行计划 → 执行命令"三阶段流程。
